...
func (s *Service) playBackendAudio(pcm []byte) {
	...
	err := s.audio.PlayPCM(sessionID, pcm, func(_ []byte, _ float64) {})
...
func (s *Service) HandleAvatarMotion(raw string) {
	...
	if seq, aa, blink, ok := avatarMotionDebugValues(payload); ok && (aa >= 0.03 || blink >= 0.25) {
		if seq == 0 || seq%10 == 0 || aa >= 0.2 {
			s.logger.Infof("assistant", "avatar motion frame seq=%d aa=%.3f blink=%.3f", seq, aa, blink)
		}
	}
...
func avatarMotionDebugValues(payload map[string]any) (seq int, aa float64, blink float64, ok bool) {
	seqValue, hasSeq := payload["sequence"]
	expressionsValue, hasExpressions := payload["expressions"]
	if !hasSeq || !hasExpressions {
		return 0, 0, 0, false
	}
	seqFloat, ok := anyFloat(seqValue)
	if !ok {
		return 0, 0, 0, false
	}
	expressions, ok := expressionsValue.(map[string]any)
	if !ok {
		return 0, 0, 0, false
	}
	aaValue, okAA := anyFloat(expressions["aa"])
	blinkValue, okBlink := anyFloat(expressions["blink"])
	if !okAA && !okBlink {
		return 0, 0, 0, false
	}
	return int(seqFloat), aaValue, blinkValue, true
}
