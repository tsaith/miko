...
def resolve_model_path() -> Path:
    ...
    if settings.models_dir is not None:
        candidates.append(settings.models_dir / "yolov12n-face.pt")
...
def create_detector(conf_threshold: float, image_size: int) -> YoloFaceDetector:
...
def observe_face(detector: YoloFaceDetector, frame) -> DetectionObservation:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="YOLO face detector webcam experiment")
    ...
    parser.add_argument("--title", default="Miko YOLO Face Detector", help="OpenCV window title")
    args = parser.parse_args()

    model_path = resolve_model_path()
    detector = create_detector(args.conf, args.imgsz)
    ...
    try:
        while True:
            ...
            if observation.has_face and observation.face_rect is not None:
                ...
                draw_osd_text(frame, f"{observation.confidence:.2f}", (x, max(24, y - 10)))

            fps_line = f"FPS: {stats.fps:.1f}  Detect: {stats.detect_ms:.2f} ms"
            pos_line = "Face: --"
            if observation.has_normalized_position:
                pos_line = (
                    f"Face: x={observation.normalized_x:.3f}  "
                    f"y={observation.normalized_y:.3f}  conf={observation.confidence:.2f}"
                )
            state_line = f"Detected: {str(observation.has_face).lower()}"
